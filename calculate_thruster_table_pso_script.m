function calculate_thruster_table_pso_script(solution_name, Thruster_Params)
...
Thruster_Params_out = Thruster_Params;
...
Fr_target_table(4,:) = [0 0 0 1 0 0]; %1N in the x direction
Fr_target_table(5,:) = [0 0 0 0 1 0]; %1N in the x direction
Fr_target_table(6,:) = [0 0 0 0 0 1]; %1N in the x direction
...
F_out = F_target;
Efficiency_out = zeros(Solution_Count,1);
...
while(index<Solution_Count)
    ...
    [Fr_out, F_out, fval_out(index)] = pso_solve_forces(Thruster_Params, Fr_target, F_target);
    %Add outputs to table
    Fr_output_table(index,:) = Fr_out';
    F_output_table(index,:) = F_out';
    Efficiency_out(index,:) = sum(abs(Fr_out))/sum(abs(F_out));
end
...
Fr_output_table = [Solution_Number Fr_output_table Fr_target_table Efficiency_out fval_out];

clock_time = clock;
date_time = date;
filename_Fr = [date_time ' ' num2str(clock_time(4)) num2str(clock_time(5)) ' ' solution_name ' Fr pso.csv'];
headers_Fr = {'Solution', 'Frx', 'Fry', 'Frz', 'Mrx', 'Mry', 'Mrz', 'Frx target', 'Fry target', 'Frz target', 'Mrx target', 'Mry target', 'Mrz target', 'Efficiency', 'fval'};
csvwrite_with_headers(filename_Fr, Fr_output_table, headers_Fr);

%F
F_output_table = [Solution_Number F_output_table F_target_table Efficiency_out fval_out];
filename_F = [date_time ' ' num2str(clock_time(4)) num2str(clock_time(5)) ' ' solution_name ' F pso.csv'];
headers_F = {'Solution', 'F1', 'F2', 'F3', 'F4', 'F5', 'F6', 'F1 target', 'F2 target', 'F3 target', 'F4 target', 'F5 target', 'F6 target', 'Efficiency', 'fval'};
csvwrite_with_headers(filename_F, F_output_table, headers_F);

%Thruster Setup
filename_Fsetup = [date_time ' ' num2str(clock_time(4)) num2str(clock_time(5)) ' ' solution_name ' Fsetup pso.mat'];
